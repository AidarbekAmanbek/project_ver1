...
from calculations.calc_punching_shear import (
    ...
    calc_punching_shear_stress_max,
    input_row,
    select_row,
)
...
with st.container(border=True):
    # -------------------------------------- расчет для ПРЯМОУГОЛЬНЫХ колонн --------------------------------------
    if column_types == "rectangular" and betta_check != "formula":
        with st.container(border=True):
        ...
        # -------------------------------------- проверка у ГРАНИ колонны --------------------------------------
        with st.container(border=True):
            u0, v_Ed0, nu, f_cd, V_Rdmax = calc_punching_shear_stress_max(
                c1, c2, vertical_shear_force, betta, d_eff, concrete, a_cc, y_c
            )
            st.write("Периметр у грани колонны $u_0$:")
            st.latex(fr"u_0 = 2\left(c_1 + c_2\right) = 2\left({c1} + {c2}\right) = {u0}\ \mathrm{{мм}}")

            st.write("Напряжение среза у грани колонны по (6.38):")
            st.latex(
                fr"v_{{Ed,0}} = \dfrac{{\beta\cdot V_{{Ed}}}}{{u_0\cdot d}}"
                fr" = \dfrac{{{betta}\cdot {vertical_shear_force}\cdot 10^3}}{{{u0}\cdot {d_eff}}} = {v_Ed0}\ \mathrm{{МПа}}"
            )

            st.write("Максимальное сопротивление продавливанию у грани колонны по (6.53):")
            st.latex(
                fr"\nu = 0.6\left(1 - \dfrac{{f_{{ck}}}}{{250}}\right)"
                fr" = 0.6\left(1 - \dfrac{{{f_ck}}}{{250}}\right) = {nu}"
            )
            st.latex(
                fr"f_{{cd}} = \dfrac{{\alpha_{{cc}}\cdot f_{{ck}}}}{{\gamma_c}}"
                fr" = \dfrac{{{a_cc}\cdot {f_ck}}}{{{y_c}}} = {f_cd}\ \mathrm{{МПа}}"
            )
            st.latex(
                fr"V_{{Rd,max}} = 0.5\cdot \nu\cdot f_{{cd}} = 0.5\cdot {nu}\cdot {f_cd} = {V_Rdmax}\ \mathrm{{МПа}}"
            )

            st.write("Проверка условия по (6.4.3(2)) — прочность бетонного подкоса у грани колонны:")
            if v_Ed0 <= V_Rdmax:
                st.latex(fr"v_{{Ed,0}} = {v_Ed0}\ \mathrm{{МПа}} \leq V_{{Rd,max}} = {V_Rdmax}\ \mathrm{{МПа}}")
                st.success("Условие выполняется — прочность бетона у грани колонны достаточна.")
            else:
                st.latex(fr"v_{{Ed,0}} = {v_Ed0}\ \mathrm{{МПа}} > V_{{Rd,max}} = {V_Rdmax}\ \mathrm{{МПа}}")
                st.error("Условие не выполняется — необходимо увеличить размеры сечения колонны/класс бетона или толщину плиты.")


        # -------------------------------------- проверка КОНТРОЛЬНОМУ ПЕРИМЕТРУ --------------------------------------
        with st.container(border=True):
            if betta_check == "formula":
                ...
                st.write("Расчет по контрольному периметру $u_1$ (на расстоянии $2d$ от грани прямоугольной колонны):")
                st.latex(
        ...
        with st.container(border=True):
            ...
            st.latex(
                fr"k = \min\left(1 + \sqrt{{\dfrac{{200}}{{d}}}},\ 2.0\right)"
                fr" = \min\left(1 + \sqrt{{\dfrac{{200}}{{{d_eff}}}}},\ 2.0\right) = {k}"
            )
            ...
            if betta_check != "formula" and rebar_include != "with_shear_rebar":
        ...
        st.info("Расчет еще не реализован")
    else:
        st.info("Расчет еще не реализован")
